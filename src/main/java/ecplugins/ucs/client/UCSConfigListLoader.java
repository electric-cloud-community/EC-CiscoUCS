...
// UCSConfigListLoader.java --
//
// UCSConfigListLoader.java is part of ElectricCommander.
//
// Copyright (c) 2005-2012 Electric Cloud, Inc.
// All rights reserved.
//

package ecplugins.ucs.client;
...
public class UCSConfigListLoader
    extends Loader
{

    //~ Instance fields --------------------------------------------------------

    private final UCSConfigList   m_configList;
    private final CgiRequestProxy m_cgiRequestProxy;
    private String                m_editorName;

    //~ Constructors -----------------------------------------------------------

    public UCSConfigListLoader(
            UCSConfigList   configList,
            Component       component,
    ...
    private void loadConfigs(Map<String, String> cgiParams)
    {
        ...
        try {
                    ...
                    new RequestCallback() {
                                ...
                                Throwable exception)
                        {
                            ...
                            ((HasErrorPanel) m_component).addErrorMessage(
                                "Error loading UCS configuration list: ",
                                exception);
                        }
            ...
            if (m_component instanceof HasErrorPanel) {
                ...
                // noinspection HardCodedStringLiteral
                ((HasErrorPanel) m_component).addErrorMessage(
                    "Error loading UCS configuration list: ", e);
            }
            else {
                m_component.getLog()
                           .error(e);
    ...
    private void loadEditors()
    {
        ...
        request.setPropertyName("/plugins/EC-CiscoUCS/project/ui_forms/"
                + m_editorName);
        request.setExpand(false);
        request.setCallback(new EditorLoaderCallback("ucscfg"));
        ...
        implements PropertyCallback
    {
        ...
        @Override public void handleResponse(Property response)
        {
            ...
            String errorMsg = "Editor '" + m_editorName
                    + "' not found for Cisco UCS plugin '" + m_configPlugin
                    + "'";
